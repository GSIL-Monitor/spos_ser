...
// SynthesizeText synthesizes plain text and saves the output to outputFile.
func SynthesizeTextToStream(text string, lang string, gender int32, audio_code int32) ([]byte, error) {
	ctx := context.Background()

	client, err := texttospeech.NewClient(ctx)
	if err != nil {
		fmt.Printf("%s: TTS failed to create client:  %v\n", ulog.CreateDateString(), err)
		return nil, err
	}
	...
	req := texttospeechpb.SynthesizeSpeechRequest{
		...
		Voice: &texttospeechpb.VoiceSelectionParams{
			//			LanguageCode: "en-US",
			//SsmlGender:   texttospeechpb.SsmlVoiceGender_FEMALE,     //MALE 1; FEMALE 2; NEUTRAL 3
			LanguageCode: lang,
			SsmlGender:   texttospeechpb.SsmlVoiceGender(gender), //MALE 1; FEMALE 2; NEUTRAL 3
		},
		AudioConfig: &texttospeechpb.AudioConfig{
			//			AudioEncoding: texttospeechpb.AudioEncoding_MP3,   //LINEAR16 1; MP3 2; OGG_OPUS 3
			AudioEncoding: texttospeechpb.AudioEncoding(audio_code), //LINEAR16 1; MP3 2; OGG_OPUS 3
		},
	}

	resp, err := client.SynthesizeSpeech(ctx, &req)
	if err != nil {
		fmt.Printf("%s: TTS failed to SynthesizeSpeech:  %v\n", ulog.CreateDateString(), err)
		return nil, err
	}
